...
export default function Location() {
  const [location, getLocation, pending] = useActionState(() => RuntimeClient.runPromise(action), null);
  return (
    <div>
      <button disabled={pending} onClick={() => startTransition(getLocation)}>
        Get Location
      </button>
      <p>Location: {location !== null ? `${location.coords.latitude} ${location.coords.longitude}` : "-"}</p>
    </div>
  );
}
